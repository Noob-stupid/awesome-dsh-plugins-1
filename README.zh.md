...
- [dsh-toolkit](https://github.com/omdsh-dev/dsh-toolkit) — 零依赖工具十件套（time/encoding/json/calculator/csv/regex/markdown/diff/stat/schema）一键安装 ⭐11 · `dsh plugin add @deepseek-ai/dsh-toolkit`
- [dsh-tool-calculator](https://github.com/omdsh-dev/dsh-tool-calculator) — 安全的数学表达式求值器，零依赖递归下降解析器 ⭐3 · `dsh plugin add @deepseek-ai/dsh-tool-calculator`
- [dsh-tool-csv](https://github.com/omdsh-dev/dsh-tool-csv) — CSV 解析/查询/统计/转换（RFC 4180） ⭐3 · `dsh plugin add @deepseek-ai/dsh-tool-csv`
- [dsh-tool-diff](https://github.com/omdsh-dev/dsh-tool-diff) — 文本/JSON/CSV/Markdown 结构化比较与 unified diff ⭐2 · `dsh plugin add @deepseek-ai/dsh-tool-diff`
- [dsh-tool-encoding](https://github.com/omdsh-dev/dsh-tool-encoding) — base64/url/hex 编解码、常用哈希、UUID 生成 ⭐2 · `dsh plugin add @deepseek-ai/dsh-tool-encoding`
- [dsh-tool-json](https://github.com/omdsh-dev/dsh-tool-json) — JMESPath 子集 JSON 查询 ⭐2 · `dsh plugin add @deepseek-ai/dsh-tool-json`
- [dsh-tool-markdown](https://github.com/omdsh-dev/dsh-tool-markdown) — HTML↔Markdown 转换、GFM 表格规范化、目录生成 ⭐2 · `dsh plugin add @deepseek-ai/dsh-tool-markdown`
- [dsh-tool-regex](https://github.com/omdsh-dev/dsh-tool-regex) — 正则测试/提取/安全替换/静态解释（不执行代码） ⭐2 · `dsh plugin add @deepseek-ai/dsh-tool-regex`
- [dsh-tool-schema](https://github.com/omdsh-dev/dsh-tool-schema) — JSON Schema 验证：validate/paths/explain/normalize ⭐2 · `dsh plugin add @deepseek-ai/dsh-tool-schema`
- [dsh-tool-stat](https://github.com/omdsh-dev/dsh-tool-stat) — 描述统计/百分位数/频数分布/相关性 ⭐2 · `dsh plugin add @deepseek-ai/dsh-tool-stat`
- [dsh-tool-time](https://github.com/omdsh-dev/dsh-tool-time) — 严格 ISO 8601 解析、IANA 时区、UTC 日历运算 ⭐2 · `dsh plugin add @deepseek-ai/dsh-tool-time`
...
- [dsh-custom-tool](https://github.com/omdsh-dev/dsh-custom-tool) — 用 Monaco 编辑器创建/管理沙箱化自定义 JS 工具 ⭐18 · `dsh plugin add dsh-custom-tool`
- [dsh-bash-encoding](https://github.com/lhh010/dsh-bash-encoding) — 自动识别并解码 Bash 输出编码（UTF-16LE/UTF-8/GBK），修中文乱码
- [dsh-at-file](https://github.com/omdsh-dev/dsh-at-file) — Codex 风格 `@file` 文件引用，输入框里直接搜索并引用工作区文件 ⭐25 · `dsh plugin add dsh-at-file`
...
- [dsh-data-agent](https://github.com/omdsh-dev/dsh-data-agent) — 让 AI 帮你连数据库、写 SQL ⭐4 · `dsh plugin add @deepseek-ai/dsh-data-agent`
...
- [dsh-cowork](https://github.com/Jesse-njx/dsh-cowork) — doc_read/doc_write：以有界、单元格寻址方式读写 xlsx/pdf/docx/pptx/ipynb
- [dsh-plugin-mineru](https://github.com/HuanLinOTO/dsh-plugin-mineru) — 向模型暴露 MineRU 文档解析工具 ⭐4 · `dsh plugin add @huanlin/dsh-plugin-mineru`
...
- [DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar) — 侧边栏完整工作台：文件渲染编辑/终端/Git/子代理，支持三方注册 Tab ⭐123 · `dsh plugin add dsh-better-sidebar`
- [dsh-side-panel](https://github.com/ccq1/dsh-side-panel) — 侧边栏集成文件浏览器、终端和 Git 审查 ⭐8 · `dsh plugin add @dsh-external/dsh-side-panel`
- [dsh-focus-chat](https://github.com/dingyi222666/dsh-focus-chat) — 「聚焦会话」精简视图，只关注最终产出结果 ⭐7 · `dsh plugin add @dingyi222666/dsh-focus-chat`
- [ui-status-label](https://github.com/alingalingling/ui-status-label) — 把鲸鱼娘思考时的 "deep diving" 状态文案自定义 ⭐21 · `dsh plugin add dsh-ui-status-label`
- [dsh-navbar](https://github.com/vlln/dsh-navbar) — 对话节点导航条，右缘节点串快速跳转 user 消息 ⭐3 · `dsh plugin add @dsh-external/dsh-navbar`
- [dsh-task-status](https://github.com/vlln/dsh-task-status) — 后台任务状态条：对话页任务进度 + 实时输出 tail ⭐3 · `dsh plugin add @dsh-external/dsh-task-status`
- [dsh-web-archive](https://github.com/renat3u/dsh-web-archive) — 折叠对话中的 Think、Bash 等「无用消息」 ⭐2 · `dsh plugin add dsh-web-archive`
- [dsh-milestone](https://github.com/SnowCrescenter-tech/dsh-milestone) — 会话里程碑导航条：像 Git 提交图定位每条提问 ⭐3 · `dsh plugin add dsh-milestone`
- [dsh-spotlight](https://github.com/0xsline/dsh-spotlight) — 键盘优先的命令面板（command palette） ⭐1 · `dsh plugin add @dsh-external/dsh-spotlight`
...
- [dsh-diff-viewer](https://github.com/lehhair/dsh-diff-viewer) — PiUI 风格 diff 查看器，替换 write/edit 的默认 DiffBlock ⭐1 · `dsh plugin add @dsh-external/dsh-diff-viewer`
- [dsh-drag-and-drop](https://github.com/bill9109/dsh-drag-and-drop) — 跨平台文件拖拽与原始路径插入，无需复制文件 ⭐1 · `dsh plugin add @bill9109/dsh-drag-and-drop`
- [ex-setting](https://github.com/omdsh-dev/ex-setting) — DSH 的设置扩展 ⭐1 · `dsh plugin add @deepseek-ai/dsh-ex-setting`
- [dsh-annotation](https://github.com/omdsh-dev/dsh-annotation) — 选中文字→批注→回车随消息发送，回复按批注逐条对照 ⭐18 · `dsh plugin add @omdsh-dev/dsh-annotation`
...
- [dsh-sticky-disclosure](https://github.com/Han-1413141/dsh-sticky-disclosure) — 把滚出屏幕的折叠标签（Think/工具卡）钉在视口顶部 ⭐1 · `dsh plugin add dsh-sticky-disclosure`
...
- [dsh-visualize](https://github.com/Nagi-ovo/dsh-visualize) — 对话内生成式 UI：模型把交互式 HTML 卡片直接画进会话流，带流式预览 ⭐29 · `dsh plugin add @dsh-external/dsh-visualize`
- [dsh-genui](https://github.com/omdsh-dev/dsh-genui) — 助手回复内渲染交互式 UI 组件：布局、图表、表单、测验、mermaid、3D 场景 ⭐14 · `dsh plugin add @omdsh-dev/dsh-genui`
...
<details>
<summary>🖥️ 桌面端 / TUI / 移动端 · 21</summary>
...
- [dsh-agent-teams](https://github.com/NanmiCoder/dsh-agent-teams) — AgentTeams 多智能体团队协作 ⭐68 · `dsh plugin add dsh-agent-teams`
- [dsh_workflow](https://github.com/icetomoyo/dsh_workflow) — 把 UltraCode 式多 Agent 调度带给 DSH：可生成/保存/治理/观察/恢复的 Workflow 层 ⭐35 · `dsh plugin add @dsh-external/workflow`
...
- [dsh-crosstalk](https://github.com/Jesse-njx/dsh-crosstalk) — 跨会话消息互发：本机任意会话像 Claude Code 一样互发消息 · `dsh plugin add @dsh-crosstalk/bundle`
...
- [dsh-interconnect](https://github.com/Chinesezjc/dsh-interconnect) — 跨实例消息/事件交接（interconnect 服务 + 工具） ⭐14 · `dsh plugin add dsh-interconnect`
...
- [dsh-mnemon](https://github.com/omdsh-dev/dsh-mnemon) — Mnemon 本地三层记忆（Runtime Memory/可检索文档/受监督 Memory Spaces） ⭐3 · `dsh plugin add dsh-mnemon`
- [nowledge-mem-deepseek-harness](https://github.com/nowledge-co/nowledge-mem-deepseek-harness) — 给所有 AI 工具共用的一层记忆（Context Bundle 注入 + MCP 工具 + 线程捕获） ⭐2 · `dsh plugin add nowledge-mem-deepseek-harness`
- [dsh-plugin-meta-memory](https://github.com/YYTbit/dsh-plugin-meta-memory) — 结构化长期记忆系统 ⭐1 · `dsh plugin add dsh-plugin-meta-memory`
- [dsh-kb-sieve](https://github.com/omdsh-dev/dsh-kb-sieve) — 从 md/txt/docx/pdf 构建可审计知识库包（SQLite FTS5） ⭐1 · `dsh plugin add @dsh-external/dsh-kb-sieve`
...
- [dsh-context-doctor](https://github.com/Zhenyu98/dsh-context-doctor) — 上下文注入审计：统计指令链/技能目录/工具 schema 的 token 成本，检测重复冲突 ⭐5 · `dsh plugin add dsh-context-doctor`
- [context-vista](https://github.com/GooodWei/context-vista) — `/context` 命令 + 环形图实时展示上下文 token 用量与费用
- [distill](https://github.com/LoserFox/distill) — 自动对话蒸馏：后台 subagent 反省 + 技能 create/update ⭐12 · `dsh plugin add @loserfox/distill`
...
- [dsh-turn-rewind](https://github.com/Anionex/dsh-turn-rewind) — 对话回退：基于持久 Change Ledger 回滚会话与工作区状态 ⭐22 · `dsh plugin add @dsh-external/turn-rewind`
...
<details>
<summary>👁️ 多模态 / 视觉 · 16</summary>

- [modlens](https://github.com/liustack/modlens) — DSH 首个视觉插件：粘贴图片返回结构化 JSON 证据（OCR/布局/语义） ⭐696 · `dsh plugin add @liustack/modlens`
- [dsh-vision-toolkit](https://github.com/Anionex/dsh-vision-toolkit) — 纯文本模型的视觉工具箱：图片问答、长截图 OCR、UI 还原、定位、像素对比、Artifacts ⭐144 · `dsh plugin add @dsh-external/dsh-vision-toolkit`
...
- [dsh-vision-proxy](https://github.com/Flyvhidbwo/dsh-vision-proxy) — DeepSeek 大脑 + 自动识图（图片经 Qwen VLM 转文字后作答） ⭐3 · `dsh plugin add dsh-vision-proxy`
...
- [Qwen-MM-Plugins](https://github.com/omdsh-dev/Qwen-MM-Plugins) — Qwen 多模态插件支持 ⭐1 · `dsh plugin add @deepseek-ai/dsh-qwen-mm`
- [dsh-computer-use](https://github.com/Anionex/dsh-computer-use) — macOS 电脑控制：Accessibility 观测、过期状态拒绝、作用域权限、安全输入 ⭐12 · `dsh plugin add @dsh-external/dsh-computer-use`
...
- [dsh-deep-research](https://github.com/omdsh-dev/dsh-deep-research) — 自适应深度研究编排器（基于官方 workflow 引擎） ⭐4 · `dsh plugin add @dsh-external/dsh-deep-research`
...
- [dsh-loop](https://github.com/vlln/dsh-loop) — 定时循环：`/loop` 命令 + loop 工具 + 活动状态条 ⭐2 · `dsh plugin add @dsh-external/dsh-loop`
- [dsh-sentinel](https://github.com/fuhefei/dsh-sentinel) — 条件驱动唤醒：file/command/http/process/webhook 持久监视触发 agent ⭐3 · `dsh plugin add @dsh-external/dsh-sentinel`
- [dsh-automation](https://github.com/titanwings/dsh-automation) — 定时任务：Coding 任务按计划在全新 Agent Session 中运行 ⭐6 · `dsh plugin add @dsh-external/dsh-automation`
...
- [dsh-plannotator](https://github.com/titanwings/dsh-plannotator) — 计划批注：选中计划原文逐条批注并回送结构化反馈 ⭐2 · `dsh plugin add @dsh-external/dsh-plannotator`
- [dsh-inspect](https://github.com/omdsh-dev/dsh-inspect) — 发现问题→修复→复查的对抗式闭环（基于官方 workflow 引擎） ⭐2 · `dsh plugin add @dsh-external/dsh-inspect`
- [dsh-advisor](https://github.com/btspoony/dsh-advisor) — 副模型每轮被动审查并注入见解 ⭐2 · `dsh plugin add dsh-advisor`
- [mstar-harness](https://github.com/btspoony/mstar-harness) — Skill 驱动的 Harness/Loop 工程工作流 Agent 插件
- [dsh-llm-fallbacks](https://github.com/btspoony/dsh-llm-fallbacks) — 基于角色的模型重试/备用策略 ⭐1 · `dsh plugin add dsh-llm-fallbacks`
...
- [dsh-track](https://github.com/fakechris/dsh-track) — 嵌入式任务管理引擎：决策点协议、念头捕获墙、Linear 形 issue 存储 ⭐2 · `dsh plugin add @deepseek-ai/dsh-track`
...
- [dsh-lark-bridge](https://github.com/imetn/dsh-lark-bridge) — 双向飞书控制器 ⭐1 · `dsh plugin add dsh-lark-bridge`
- [dsh-onlyne](https://github.com/dbydd/dsh-onlyne) — IM 网关：从 dsh 会话收发 QQ/微信/飞书/Telegram 消息
- [dsh-notification](https://github.com/omdsh-dev/dsh-notification) — 回合完成桌面通知，按结果分控 + 关键词过滤 ⭐23 · `dsh plugin add dsh-notification`
...
- [dsh-web-ui-notify](https://github.com/bill9109/dsh-web-ui-notify) — 桌面通知提醒 ⭐3 · `dsh plugin add @bill9109/dsh-web-ui-notify`
- [dsh-session-notification](https://github.com/dingyi222666/dsh-session-notification) — 会话完成等四种状态通知，支持浏览器提示 ⭐2 · `dsh plugin add @dingyi222666/dsh-session-notification`
...
- [dsh-open-in-vscode](https://github.com/omdsh-dev/dsh-open-in-vscode) — 从 Web GUI 一键在 VS Code 中打开工作区目录 ⭐33 · `dsh plugin add dsh-open-in-vscode`
- [dsh-share](https://github.com/hellodigua/dsh-share) — 一键分享你的对话 ⭐11 · `dsh plugin add @dsh-external/dsh-share`
...
- [dsh-browser](https://github.com/Lum1104/dsh-browser) — Chrome 侧边栏扩展，让 DSH 直接操作你的浏览器（无需视觉能力）
...
- [dsh-webbridge](https://github.com/bill9109/dsh-webbridge) — DSH 结合 Kimi WebBridge ⭐1 · `dsh plugin add @bill9109/dsh-webbridge`
...
- [dsh-web-search-pro](https://github.com/anweat/dsh-web-search-pro) — 增强持久搜索（多引擎 + SQLite/LRU 缓存 + Playwright 渲染） ⭐1 · `dsh plugin add dsh-web-search-pro`
...
<details>
<summary>🏗️ 基础设施 / 插件管理 / 开发工具 · 30</summary>
...
- [plugin-registry](https://github.com/vlln/plugin-registry) — 插件管理控制台：浏览器面板管理官方 repository 插件 + 开发引导
...
- [dsh-plugin-check](https://github.com/omdsh-dev/dsh-plugin-check) — 插件健康检查：扫描清单协议/patch 格式/构建陷阱/hub 状态 ⭐11 · `dsh plugin add @deepseek-ai/dsh-plugin-check`
...
- [dsh-security-audit](https://github.com/omdsh-dev/dsh-security-audit) — 本机安全审计：配置/插件来源/会话/网络暴露面，只读脱敏报告 ⭐8 · `dsh plugin add @deepseek-ai/dsh-security-audit`
- [dsh-session-health](https://github.com/omdsh-dev/dsh-session-health) — 会话文件帧级扫描诊断（torn/损坏/空会话检测） ⭐7 · `dsh plugin add @deepseek-ai/dsh-session-health`
- [dsh-evolve](https://github.com/william-jin-cmu/dsh-evolve) — 自进化：agent 会话内给自己热挂载/卸载持久化插件 ⭐2 · `dsh plugin add @dsh-external/dsh-evolve`
...
- [fabric](https://github.com/omdsh-dev/fabric) — 类似 MC Fabric 的 hook 处理器 ⭐7 · `dsh plugin add cordis-fabric-bundle`
- [sandbox-micro](https://github.com/omdsh-dev/sandbox-micro) — microsandbox 沙箱支持 ⭐2 · `dsh plugin add @deepseek-ai/dsh-sandbox-microsandbox`
- [sandbox-mxc](https://github.com/omdsh-dev/sandbox-mxc) — 微软跨平台沙盒支持 ⭐1 · `dsh plugin add @deepseek-ai/dsh-sandbox-mxc`
- [sandbox-nono](https://github.com/omdsh-dev/sandbox-nono) — nono 沙盒支持 ⭐1 · `dsh plugin add @deepseek-ai/dsh-sandbox-nono`
...
- [dsh-multica-runtime](https://github.com/forrestchang/dsh-multica-runtime) — 支持在 Multica 上使用 DSH 作为 runtime ⭐6 · `dsh plugin add @multica-ai/dsh-runtime`
...
<details>
<summary>🎮 娱乐 / 其他 · 27</summary>

- [dsh-gomoku](https://github.com/omdsh-dev/dsh-gomoku) — 与 AI 下五子棋，也可双 AI 对弈比棋力 ⭐6 · `dsh plugin add @deepseek-ai/dsh-gomoku`
- [dsh-minigames](https://github.com/lhh010/dsh-minigames) — 右侧 18 款离线小游戏面板（恐龙跳一跳/俄罗斯方块/扫雷/2048…） ⭐6 · `dsh plugin add @dsh-external/dsh-minigames`
- [dsh-auto-chess](https://github.com/omdsh-dev/dsh-auto-chess) — 自走棋：人机对战或双 AI 对弈 ⭐1 · `dsh plugin add @deepseek-ai/dsh-auto-chess`
- [dsh-plugin-d399](https://github.com/HuanLinOTO/dsh-plugin-d399) — 模型生成时弹出小游戏菜单（wordle/消消乐，可扩展） ⭐1 · `dsh plugin add @huanlin/dsh-plugin-d399`
- [dsh-ui-whale](https://github.com/lhh010/dsh-ui-whale) — 全手绘像素鲸鱼伙伴（眨眼/摆尾/喷水/爱心）
- [whale-girl](https://github.com/vlln/whale-girl) — 桌面宠物鲸鱼娘（QQ 宠物形态，可拖拽/投喂/玩耍） ⭐25 · `dsh plugin add whale-girl`
...
- [dsh-stickers](https://github.com/william-jin-cmu/dsh-stickers) — 用户与 agent 双向表情贴纸互动 ⭐6 · `dsh plugin add @dsh-external/dsh-stickers`
- [dsh-emoji](https://github.com/hellodigua/dsh-emoji) — 为 AI 回复自动添加表情 ⭐7 · `dsh plugin add @dsh-external/dsh-emoji`
- [dsh-ads](https://github.com/Nagi-ovo/dsh-ads) — 2005 中文站点风格整活广告（侧栏/信息流/弹窗，素材全虚构） ⭐103 · `dsh plugin add @dsh-external/dsh-ads`
- [dsh-stock-market](https://github.com/AnacondaKC/dsh-stock-market) — 股票行情数据插件（整活向） ⭐4 · `dsh plugin add dsh-stock-market`
- [dsh-douyin](https://github.com/AnacondaKC/dsh-douyin) — 侧栏短视频：原生播放器、系列导航、历史回放 ⭐2 · `dsh plugin add dsh-douyin`
- [deepseek-manners](https://github.com/Moeblack/deepseek-manners) — 每次消息后注入感谢语，做个有礼貌的人 ⭐2 · `dsh plugin add deepseek-manners`
...
- [dsh-openmaic](https://github.com/THU-MAIC/dsh-openmaic) — OpenMAIC 教学：课堂、幻灯片、交互组件、苏格拉底式教学 ⭐3 · `dsh plugin add @openmaic/dsh-openmaic`
- [dsh-scholar](https://github.com/lzszq/dsh-scholar) — 学术助手插件 ⭐3 · `dsh plugin add @dsh-scholar/research-plugin`
- [dsh-101](https://github.com/bill9109/dsh-101) — DSH 文档阅读模式 ⭐1 · `dsh plugin add @dsh-external/dsh-101`
